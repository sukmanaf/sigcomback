...
interface PolygonEditModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (data: PolygonEditData) => Promise<void>;
    type: EditLayerType;
    id: string | number;
    code: string;
    coordinates: number[][];
...
export interface PolygonEditData {
    type: EditLayerType;
    id: string | number;
    code: string;
    coordinates: number[][];
...
const LAYER_COLORS: Record<EditLayerType, { from: string; to: string }> = {
    nop: { from: 'from-emerald-600', to: 'to-teal-600' },
    blok: { from: 'from-fuchsia-600', to: 'to-purple-600' },
    bangunan: { from: 'from-red-600', to: 'to-orange-600' },
};
...
// Code field labels
const CODE_LABELS: Record<EditLayerType, string> = {
    nop: 'NOP',
    blok: 'Kode Blok',
    bangunan: 'NOP Referensi',
};

export default function PolygonEditModal({
    isOpen,
    onClose,
    onSave,
    type,
    ...
    // Reset editedCode when code prop changes (modal opens with new data)
    useEffect(() => {
        setEditedCode(code);
        setNewFiles([]);
        setDeletedImages([]);
    ...
    const codeLabel = CODE_LABELS[type];
        ...
        <>
            ...
            <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className={`bg-gradient-to-r ${colors.from} ${colors.to} text-white px-6 py-5 flex items-center justify-between shrink-0`}>
                    ...
                    <div className="p-6 space-y-5 overflow-y-auto flex-1">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder={`Masukkan ${codeLabel}`}
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 shrink-0">
                        ...
                        <button
                            ...
                            disabled={isSubmitting}
                            className={`px-5 py-2.5 bg-gradient-to-r ${colors.from} ${colors.to} text-white rounded-xl hover:opacity-90 transition-all disabled:opacity-50 font-medium flex items-center gap-2 shadow-lg`}
                        >
                            {isSubmitting ? (
                                <>
            ...
            {/* Image Preview Lightbox - Only for NOP */}
            {type === 'nop' && previewIndex !== null && (
                <div
