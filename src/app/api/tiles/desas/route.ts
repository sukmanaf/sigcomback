...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Higher zoom = more detail, lower zoom = simpler geometry
    const tolerance = Math.pow(2, 20 - z) / 111320; // Convert to degrees
    
    // Query features within tile bounds with simplified geometry
    const features = await db
      .select({
        id: desas.id,
        d_kd_kel: desas.d_kd_kel,
        d_nm_kel: desas.d_nm_kel,
        geom: sql`ST_AsGeoJSON(ST_SimplifyPreserveTopology(${desas.geom}, ${tolerance}))`,
      })
      .from(desas)
      .where(sql`ST_Intersects(ST_SetSRID(${desas.geom}, 4326), ST_GeomFromText(${bbox}, 4326))`)
      .limit(100); // Limit to 100 desas per tile for performance
    ...
    // Compress response for smaller payload
