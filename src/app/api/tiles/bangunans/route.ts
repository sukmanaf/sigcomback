  ...
  try {
    ...
    const features = await db
      .select({
        ...
        geom: sql`ST_AsGeoJSON(ST_SimplifyPreserveTopology(${bangunans.geom}, ${tolerance}))`,
      })
      .from(bangunans)
      .where(
        sql`ST_Intersects(ST_SetSRID(${bangunans.geom}, 4326), ST_GeomFromText(${bbox}, 4326)) 
            AND SUBSTRING(${bangunans.d_nop}, 1, 10) = ${desaKode}`
      )
      .limit(500);
